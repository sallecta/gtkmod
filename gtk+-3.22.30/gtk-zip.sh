...
ZIP=/tmp/gtk+-3.22.30.zip
DEVZIP=/tmp/gtk+-dev-3.22.30.zip

SHORTAPIVERSION=`echo 3.0 | tr -d '.'`
APIVERSIONMAJOR=`echo 3.0 | sed -e 's/\..*//'`

cd /tmp/gtk32230build

mkdir -p share/doc/gtk+-3.22.30
cp -p /media/data/dev/git/gtk32230/gtk+-3.22.30/COPYING share/doc/gtk+-3.22.30

mkdir -p share/doc/gtk+-dev-3.22.30
cp -p /media/data/dev/git/gtk32230/gtk+-3.22.30/COPYING share/doc/gtk+-dev-3.22.30
...
zip -r -D $ZIP share/doc/gtk+-3.22.30
...
zip -r $DEVZIP share/doc/gtk+-dev-3.22.30
